...
import config
import network
from config import SCREEN, BACKGROUND, WIDTH, HEIGHT, CLOCK, FPS, YELLOW, GAME_SPEED, LIVE
from reset import *
from draw import *
from player import *
...
while running:
    ...
    if config.round_finished:
        ...
        if config.online_mode:
            ...
            draw_opponent_obstacle(config.opponent_obstacle, config.scale, config.opp_offset)
    ...
    if not config.game_started:
        ...
        if config.countdown_start_time is None: # 是不是同一個物件
            if not config.online_mode:
                ...
                CLOCK.tick(FPS)
                continue
        remaining = config.countdown_seconds - ((pygame.time.get_ticks() - config.countdown_start_time) // 1000)
        SCREEN.fill(BACKGROUND)
        if remaining > 0:
    ...
    config.distance += config.game_speed * (CLOCK.tick(FPS) / 1000) # 一幀的秒數 * 速率 = 一幀的距離
    ...
    if config.online_mode and config.client_socket and not config.round_finished:
        if now - config.last_send_time >= 1: # 每 1ms 傳送一次
            network.send_state(config.client_socket, player, config.obstacle_group, config.points, config.buff_group)
